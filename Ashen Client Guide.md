...
> Often impacts Custom Models, and HD patches

You need to enable the Vertex Animation Shaders option in video options, the client must be restarted afterward to take effect. [Video](https://youtu.be/7_3kaPjQn-Y) [Screenshots](https://imgur.com/a/7zQRxNW)
...
### 3-F. [PerfBoost (Mirror)](https://github.com/tubtubs/VanillaWoWClientModBackup/tree/main/ClientMods/perf_boost) [Original Repo (dead)](https://gitea.com/avitasia/perf_boost) Deadlink
...
In the wow folder you should now have a `dlls.txt`, open it and add the DLLs you want supported to it. Some are already listed at the top, like Nampower and SuperWoW. Here's an example of what I might end up adding to my `dlls.txt`. If you launch and one of your addons complains about a missing client mod try removing the dll from the list, and putting it at the top or otherwise changing the order. I haven't narrowed it down, but some client mods may wipe other client mods changes to the lua api for example.
```
...
### 3-N. [DXVK](https://github.com/doitsujin/dxvk/releases) (optional / only if you need it)
...
### 3-O. [Vanilla MultiMonitor Fix](https://github.com/Mates1500/VanillaMultiMonitorFix/releases) (optional / only if you need it)
...
Big addon lists:
* [TubsAddonList](https://github.com/tubtubs/WoWClientSetupGuides/blob/main/Addons.md)
...
## 5. BONUS PATCHES
Patches are installed in the data folder, and must be named properly so as to avoid overwriting default patches. Anything Patch-A.mpq to Patch-Z.mpq will generally be safe. Be sure to delete or disable WDB when attempting these patches.
...
### [Project Reforged (HD Patch)](https://projectreforged.github.io/)
I haven't tested this much personally. Check compatability before using any other patches with this one, it bundles many other patches. *Must* install VanillaHelpers for this patch to work properly.
...
This is a custom ported music pack from future expansions. Tuned for Vanilla.
